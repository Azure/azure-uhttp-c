...
static void setup_uhttp_client_execute_request_with_content_mocks(void)
{
    ...
    STRICT_EXPECTED_CALL(STRING_new());
    STRICT_EXPECTED_CALL(HTTPHeaders_GetHeaderCount(IGNORED_ARG, IGNORED_ARG));
    STRICT_EXPECTED_CALL(HTTPHeaders_GetHeader(IGNORED_ARG, IGNORED_ARG, IGNORED_ARG));
    STRICT_EXPECTED_CALL(gballoc_malloc(IGNORED_ARG));
    STRICT_EXPECTED_CALL(STRING_concat(IGNORED_ARG, IGNORED_ARG));
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_ARG));
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_ARG));
...
static void setup_uhttp_client_dowork_no_msg_mocks(void)
{
    ...
    STRICT_EXPECTED_CALL(singlylinkedlist_get_head_item(IGNORED_ARG)).CallCannotFail();
    STRICT_EXPECTED_CALL(singlylinkedlist_item_get_value(IGNORED_ARG)).CallCannotFail();
    ...
    STRICT_EXPECTED_CALL(STRING_delete(IGNORED_ARG));
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_ARG));
    STRICT_EXPECTED_CALL(singlylinkedlist_remove(IGNORED_ARG,IGNORED_ARG)).CallCannotFail();
...
static void setup_uhttp_client_onBytesReceived_small_ex(void)
{
    ...
    STRICT_EXPECTED_CALL(BUFFER_u_char(IGNORED_ARG));
    STRICT_EXPECTED_CALL(BUFFER_length(IGNORED_ARG));
    STRICT_EXPECTED_CALL(BUFFER_shrink(IGNORED_ARG, IGNORED_ARG, false));
    STRICT_EXPECTED_CALL(BUFFER_u_char(IGNORED_ARG));
    STRICT_EXPECTED_CALL(BUFFER_length(IGNORED_ARG));
    SetupProcessHeader();
    SetupProcessHeader();
    ...
    STRICT_EXPECTED_CALL(BUFFER_u_char(IGNORED_ARG));
    STRICT_EXPECTED_CALL(BUFFER_length(IGNORED_ARG));
    STRICT_EXPECTED_CALL(on_msg_recv_callback(IGNORED_ARG, HTTP_CALLBACK_REASON_OK, IGNORED_ARG, IGNORED_ARG, 200, IGNORED_ARG));
...
TEST_FUNCTION(uhttp_client_create_fails)
{
    ...
    EXPECTED_CALL(xio_create(IGNORED_ARG, IGNORED_ARG));
    ...
    for (size_t index = 0; index < count; index++)
    {
        ...
        // assert
        ASSERT_IS_NULL(clientHandle, "uhttp_client_create failure in test %zu/%zu", index, count);
    }
...
TEST_FUNCTION(uhttp_client_open_succeed)
{
    ...
    EXPECTED_CALL(xio_open(IGNORED_ARG, IGNORED_ARG, IGNORED_ARG, IGNORED_ARG, IGNORED_ARG, IGNORED_ARG, IGNORED_ARG));
...
TEST_FUNCTION(uhttp_client_open_fail)
{
    ...
    for (size_t index = 0; index < count; index++)
    {
        ...
        //assert
        ASSERT_ARE_NOT_EQUAL(HTTP_CLIENT_RESULT, HTTP_CLIENT_OK, httpResult, "uhttp_client_open failure in test %zu/%zu", index, count);
    }
...
TEST_FUNCTION(uhttp_client_execute_request_no_content_fails)
{
    ...
    for (size_t index = 0; index < count; index++)
    {
        if (umock_c_negative_tests_can_call_fail(index))
        {
            ...
            // assert
            ASSERT_ARE_NOT_EQUAL(HTTP_CLIENT_RESULT, HTTP_CLIENT_OK, httpResult, "uhttp_client_execute_request failure in test %zu/%zu", index, count);
        }
    }
...
TEST_FUNCTION(uhttp_client_set_X509_cert_ecc_succeed)
{
    ...
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_ARG, TEST_CERTIFICATE));
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_ARG, TEST_PRIVATE_KEY));
...
TEST_FUNCTION(uhttp_client_set_trusted_cert_malloc_fail)
{
    ...
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_ARG, TEST_CERTIFICATE)).SetReturn(__LINE__);
...
TEST_FUNCTION(uhttp_client_set_option_succeed)
{
    ...
    STRICT_EXPECTED_CALL(xio_setoption(IGNORED_ARG, TEST_SET_OPTION_NAME, TEST_SET_OPTION_VALUE));
...
TEST_FUNCTION(uhttp_client_set_option_call_fail)
{
    ...
    STRICT_EXPECTED_CALL(xio_setoption(IGNORED_ARG, TEST_SET_OPTION_NAME, TEST_SET_OPTION_VALUE)).SetReturn(1);
